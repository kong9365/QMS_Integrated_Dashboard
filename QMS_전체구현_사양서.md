...
> 확정 IA: 7 워크스페이스 · **단일 구조 + 워크스페이스별 고정 스코핑** · 연계 드릴다운 전 화면 내장 · 종결점검 분산+종합요약. (IA 맵 HTML 참조)
> **정정(역할 필터 제거)**: 기존 "단일 구조 + 역할 필터(QC/QA) 토글" → "단일 구조 + 워크스페이스별 고정 스코핑". QC/QA 는 토글이 아니라 워크스페이스 **주관 정보 라벨(배지)**로만 표기. 일탈/조사 기본 스코프: **QC 시험품질=시험실(작성팀 기준) · QA 품질운영=전사**.
...
**Task 2.2 — (삭제됨) 역할 필터(QC/QA) 토글**
- ~~상단 segmented_control 로 전체/QC/QA 토글~~ → **제거**. 기반 결정 변경: "단일 구조 + 역할 필터" → "단일 구조 + 워크스페이스별 고정 스코핑".
- 대체: 역할은 토글이 아니라 워크스페이스가 고정 보유한다. 일탈/조사는 QC 시험품질=시험실(작성팀 기준) / QA 품질운영=전사로 **워크스페이스별 고정 스코프**. QC/QA 는 주관 배지(정보 라벨)로만 표기. (Task 2.1 워크스페이스 배치 + 스코프에 흡수)

**Task 2.3 — 글로벌 필터바 상시화** (완료)
- 작업: 연도·기준(발견/등록)·진행상태·D-day 를 상단 가로 바로 이동(`st.columns`+고정 컨테이너). **('역할'은 제거 — Task 2.2 삭제 반영.)** 사이드바에 세로로 쌓이던 16개 프로젝트 건수 캡션은 상단 수집상태(총건수)로 이전.
- 완료 기준: 워크스페이스를 옮겨도 필터 선택이 유지됨(세션 상태, 위젯 key=`flt_*`). 사이드바 과밀 해소.
- **검색/Excel 보류(확정)**: 상단바 ③검색·④Excel 중 🔍검색은 **Phase 3 후보로 보류**(대상 범위 정의 필요), ↓Excel 은 **기존 데이터·설정(원본 데이터 섹션) 내보내기 사용**(상단 전역 버튼 미추가). 상단바엔 ↻갱신·↺필터초기화만 배치.
...
- 작업: 기존 `alert_service`(send_slack/send_email/run_overdue_alert) 위에 룰 관리 화면: 기한초과·D-3 임박·재발·미종결 누적 룰 + 워크스페이스/스코프별(QC 시험품질·QA 품질운영 등) 구독 + Slack/이메일 채널. (역할 토글 제거 반영 — 구독 기준은 워크스페이스 스코프.) 발송은 `refresh_job` 또는 별도 스케줄에 연결.
...
6. **좌측 레일 / 글로벌 필터바** — `streamlit-option-menu` / `st.columns`. (역할 토글 제거 — Task 2.2 삭제. `st.segmented_control` 은 아래 7번 sub-view 용으로 유지.) 상단바 작업버튼은 ↻갱신·↺필터초기화만. **🔍전역 검색은 Phase 3 후보(보류)**, **↓Excel 은 데이터·설정 원본 데이터 섹션의 기존 내보내기 사용**(상단 전역 버튼 미추가, 확정).
