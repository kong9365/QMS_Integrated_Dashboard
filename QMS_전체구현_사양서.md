...
- 작업: 연도·기준(발견/등록)·진행상태·D-day 를 상단 가로 바로 이동(`st.columns`+고정 컨테이너). **('역할'은 제거 — Task 2.2 삭제 반영.)** 사이드바에 세로로 쌓이던 16개 프로젝트 건수 캡션은 종합현황 "수집 상태 카드"로 이전.
...
6. **좌측 레일 / 글로벌 필터바** — `streamlit-option-menu` / `st.columns`. (역할 토글 제거 — Task 2.2 삭제. `st.segmented_control` 은 아래 7번 sub-view 용으로 유지.)
