...
6. **좌측 레일 / 글로벌 필터바** — `streamlit-option-menu` / `st.columns`. (역할 토글 제거 — Task 2.2 삭제. `st.segmented_control` 은 아래 7번 sub-view 용으로 유지.) 상단바 작업버튼은 ↻갱신·↺필터초기화만. **🔍통합검색은 Phase 3 후보(보류)**, **↓Excel 은 데이터·설정 원본 데이터 섹션의 기존 내보내기 사용**(상단 전역 버튼 미추가, 확정).
