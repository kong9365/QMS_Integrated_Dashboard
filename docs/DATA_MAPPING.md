...
> **종결 모수 제외 규칙(오탐 정밀화):** 위 두 플래그 판정 시 '기한일 연장' 전용 프로젝트(`list_extension`, `프로젝트`=='기한연장') 레코드는 **'실질 자식'에서 제외**한다(마감연장 종결 ≠ 본조치 종결). `qms_linkage._is_extension_record`. 표시용 `자식 수/종결률/미종결 목록` 집계는 전체 기준 유지.
...
| 재발 | ⚠️ `재발여부 == "예"` → **항상 0**(아래 재발 NOTE). deviation 단독(`deviationoutsourcing`=0행 stub). 신호 정의 재합의 전까지 잠정 |
| Analyst error | `이상발생 원인 == "Analyst error"` 건수기여도 합 (전년 대비 감소율은 `render_analyst_error_reduction_kpi` 로직) |

> **재발 NOTE (Task 3.4 디코드 — 검증된 사실 정정, 신호 로직/코드 불변)**: `재발여부` 의 실제 소스는
> **`recurrence1`** 필드(라벨 **"A. 재발가능성"** — 재발 *발생* 이 아니라 *가능성* 점수, 리스크 패널 A/B/C 의
> A항, 타입 **select**). 파서(`QMS_API.py`)는 y/n → 예/아니오 매핑하고 **그 외 값은 raw 통과** → 실데이터 값은
> `''`·`'2'`·`'6'`·`'아니오'` 로 **`'예'` 0건**. 따라서 `재발여부 == "예"` 비교는 **항상 0**. 옵션 값 `2`/`6` 의
> 라벨(value→label)은 레포의 캐시 폼 스키마·audit·문서 어디에도 **정의 없음(미확인)**. → 재발 신호는 라이브
> 폼 'A.재발가능성' 옵션 라벨 확인 후 **도메인 재정의 필요**(추측 금지). **현 신호 로직/코드는 변경하지 않음.**
...
| 재발 추세 | ⚠️ 월별 `재발여부=="예"` → 항상 0(§1 재발 NOTE 참조). `recurrence1`('A.재발가능성' select) 옵션 라벨 미확인 |
...
| 재발 발생 | ⚠️ `재발여부 == "예"` → **항상 0**(§1 재발 NOTE). 실 소스=`recurrence1`('A.재발가능성', select, 점수류). 도메인 재정의 필요 — 추측 금지 |
| 미종결 누적 | `자식 미종결 수 ≥ 3` |
